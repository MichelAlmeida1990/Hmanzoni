...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const HotelComponents = {
        ...
        setupCustomCursor: function() {
            ...
            // Lista de elementos onde o cursor personalizado deve ser exibido
            const cursorSelectors = [
                'body', 'main', 'header', 'section', 'div', 'button', 'a', 
                'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'span', 'img',
                'nav', 'ul', 'li', 'form', 'input', 'textarea', 'select',
                '.clickable', '.btn', '.card', '.room-card', '.service-card', '.gallery-item', '.testimonial'
            ].join(', ');
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                // Verificar se o elemento sob o cursor deve ter o cursor personalizado
                const elementUnderCursor = document.elementFromPoint(e.clientX, e.clientY);
                
                // Verificar se o elemento está dentro do footer
                const isInFooter = elementUnderCursor && 
                    (elementUnderCursor.closest('footer') || 
                     elementUnderCursor.closest('.footer'));
                
                if (isInFooter) {
                    // Não mostrar cursor personalizado no footer
                    cursorContainer.style.display = 'none';
                    document.documentElement.style.cursor = '';
                } else {
                    // Verificar se deve mostrar o cursor personalizado
                    const shouldShowCustomCursor = elementUnderCursor && 
                        elementUnderCursor.matches(cursorSelectors);
                    ...
                    if (shouldShowCustomCursor) {
                        ...
                        const isClickable = elementUnderCursor.matches(clickableElements) || 
                                          elementUnderCursor.closest(clickableElements);
