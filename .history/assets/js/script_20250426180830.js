...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const HotelComponents = {
        ...
        exposeToGlobalApp: function() {
            // Verificar se HotelApp existe
            if (typeof window.HotelApp !== 'undefined') {
                // Substituir a função de chatbot do HotelApp pela nossa versão avançada
        ...
        sendChatbotMessage: function(message, messagesContainer) {
            if (!messagesContainer) {
                // Tentar encontrar o container de mensagens se não for fornecido
            ...
            if (this.settings.chatbotApiEnabled && this.settings.chatbotApiKey) {
                // Usar API do DeepSeek para respostas mais avançadas
        ...
        getDeepSeekResponse: async function(message) {
            try {
                // Para teste, vamos usar uma implementação simplificada que não depende da API real
                // Isso permite testar o fluxo sem a chave da API
                
                // Simulação de resposta para teste - remova este bloco ao usar a API real
                ...
                // Extrair a resposta do modelo DeepSeek
                if (data.generated_text) {
                    return data.generated_text;
                } else if (data.outputs) {
                    return data.outputs;
                } else {
                    // Fallback para caso a estrutura da resposta seja diferente
        ...
        // Apenas os métodos essenciais para o chatbot foram mantidos
        // Os outros métodos (setupParallaxEffects, setupAdvancedGallery, etc.) continuam como estavam
        // ...
    };
    ...
    // Expor o objeto para acesso global
    window.HotelComponents = HotelComponents;
});
