        ...
        initializeMap: function(mapContainer) {
            ...
            hotelMarker.bindPopup(`
                <div class="map-popup">
                    ...
                    <a href="tel:+551155555555">(11) 5555-5555</a>
                    <a href="https://maps.google.com/?q=${hotelLat},${hotelLng}" target="_blank">Ver no Google Maps</a>
                </div>
            `);
            
            // Adicionar pontos de interesse próximos
            const pointsOfInterest = [
                { name: 'Aeroporto', lat: hotelLat + 0.01, lng: hotelLng - 0.01, type: 'transport' },
                { name: 'Shopping Center', lat: hotelLat - 0.008, lng: hotelLng + 0.005, type: 'shopping' },
                { name: 'Parque Municipal', lat: hotelLat + 0.005, lng: hotelLng + 0.01, type: 'attraction' },
                { name: 'Estação de Metrô', lat: hotelLat - 0.003, lng: hotelLng - 0.007, type: 'transport' }
            ];
            
            // Adicionar marcadores para pontos de interesse
            pointsOfInterest.forEach(poi => {
                const poiMarker = L.marker([poi.lat, poi.lng]).addTo(map);
