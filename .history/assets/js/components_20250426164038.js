...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const HotelComponents = {
        ...
        registerGlobalEvents: function() {
            ...
            // Detectar quando elementos são desabilitados dinamicamente
            document.addEventListener('change', (e) => {
                if (this.settings.cursorEnabled && e.target.disabled !== undefined) {
        ...
        setupCustomCursor: function() {
            ...
            // Ocultar cursor nativo apenas se o cursor personalizado estiver ativado
            ...
            // Elementos clicáveis para efeito hover - Sincronizado com o CSS
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                const isDisabled = target.disabled || target.classList.contains('disabled') || 
                                  (target.closest('button, input, select, .btn') && 
                                   (target.closest('button, input, select, .btn').disabled || 
                                    target.closest('button, input, select, .btn').classList.contains('disabled')));
                
                // Aplicar classe apropriada
                if (isClickable) {
                    if (isDisabled) {
                        cursorContainer.classList.remove('cursor-hover');
                        cursorContainer.classList.add('cursor-not-allowed');
                    } else {
                        cursorContainer.classList.add('cursor-hover');
                        cursorContainer.classList.remove('cursor-not-allowed');
                    }
                } else {
                    cursorContainer.classList.remove('cursor-hover');
                    cursorContainer.classList.remove('cursor-not-allowed');
            ...
            document.addEventListener('mousedown', (e) => {
                ...
                // Não aplicar efeito de clique em elementos desabilitados
                const target = e.target;
                const isDisabled = target.disabled || target.classList.contains('disabled') || 
                                  (target.closest('button, input, select, .btn') && 
                                   (target.closest('button, input, select, .btn').disabled || 
            ...
            // Método para atualizar visibilidade do cursor
            ...
            // Método para atualizar estado do cursor para um elemento específico
            this.updateCursorState = function(element) {
                if (!this.settings.cursorEnabled || !element) return;
                
                // Verificar se o elemento está desabilitado
                const isDisabled = element.disabled || element.classList.contains('disabled');
                
                // Atualizar classe do cursor se o mouse estiver sobre o elemento
            ...
            // Observar mudanças em elementos desabilitados
            if ('MutationObserver' in window) {
                const observer = new MutationObserver((mutations) => {
                    mutations.forEach((mutation) => {
                        if (mutation.type === 'attributes' && 
                            (mutation.attributeName === 'disabled' || mutation.attributeName === 'class')) {
                            this.updateCursorState(mutation.target);
                        }
                    });
                });
                
                // Observar todos os elementos interativos para mudanças de atributos
                document.querySelectorAll('button, input, select, .btn').forEach(element => {
        ...
        initializeMap: function(mapContainer) {
            ...
            const pointsOfInterest = [
                ...
                },
                {
                    ...
                    lng: hotelLng + 0.01,
                    icon: 'shopping-bag',
                    description: 'Shopping Center - 5 min a pé'
                },
                {
                    name: 'Praia',
                    lat: hotelLat + 0.01,
                    lng: hotelLng - 0.01,
                    icon: 'umbrella-beach',
                    description: 'Praia Central - 10 min a pé'
                },
                {
                    name: 'Restaurante',
                    lat: hotelLat - 0.005,
                    lng: hotelLng - 0.005,
                    icon: 'utensils',
                    description: 'Restaurantes e bares - 3 min a pé'
                }
            ];
            ...
            pointsOfInterest.forEach(poi => {
                const poiIcon = L.divIcon({
                    html: `<i class="fas fa-${poi.icon}"></i>`,
            ...
            centerButton.onAdd = function() {
                ...
                div.innerHTML = '<a href="#" title="Centralizar no Hotel" role="button" aria-label="Centralizar no Hotel"><i class="fas fa-home"></i></a>';
            ...
            // Adicionar botão para alternar entre mapa e satélite
            const satelliteButton = L.control({position: 'bottomright'});
            satelliteButton.onAdd = function() {
                const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control');
                div.innerHTML = '<a href="#" title="Alternar Visualização" role="button" aria-label="Alternar Visualização"><i class="fas fa-globe"></i></a>';
                ...
                div.onclick = function() {
                    if (isSatellite) {
                        ...
                        div.innerHTML = '<a href="#" title="Ver Satélite" role="button" aria-label="Ver Satélite"><i class="fas fa-globe"></i></a>';
                    } else {
                        map.removeLayer(streetLayer);
                        satelliteLayer.addTo(map);
                        div.innerHTML = '<a href="#" title="Ver Mapa" role="button" aria-label="Ver Mapa"><i class="fas fa-map"></i></a>';
                    }
            ...
            // Atualizar mapa quando o tema mudar
            document.addEventListener('themeChanged', (e) => {
                setTimeout(() => {
                    map.invalidateSize();
                }, 300);
