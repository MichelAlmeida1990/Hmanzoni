    ...
    const HotelManzoni = {
        ...
        setupMobileMenu: function() {
            ...
            if (mobileMenuToggle && nav) {
                ...
                // Melhorar navegação por teclado dentro do menu
                navLinks.forEach(function(link, index) {
                    link.addEventListener('keydown', function(e) {
                        if (e.key === 'Tab' && e.shiftKey && index === 0) {
                            // Se for o primeiro link e pressionar Shift+Tab, focar no botão do menu
                            e.preventDefault();
                            mobileMenuToggle.focus();
                        } else if (e.key === 'Tab' && !e.shiftKey && index === navLinks.length - 1) {
                            // Se for o último link e pressionar Tab, focar no botão do menu
                            e.preventDefault();
                            mobileMenuToggle.focus();
                        }
                    });
                ...
                console.log('Menu mobile configurado com sucesso'); // Debug
            } else {
                console.error('Elementos do menu mobile não encontrados'); // Debug
            }
        },
        ...
        registerGlobalEvents: function() {
            ...
            document.addEventListener('themeChanged', function(e) {
                ...
                self.updateComponentsTheme(e.detail.theme);
            });
            
            // Detectar redimensionamento da janela
            window.addEventListener('resize', function() {
                // Atualizar configuração do cursor com base no tamanho da tela
                self.config.cursorEnabled = window.innerWidth > 1024;
                
                // Atualizar visibilidade do cursor
                if (typeof self.updateCursorVisibility === 'function') {
            ...
            if (window.matchMedia) {
                ...
                if (!localStorage.getItem('theme')) {
                    ...
                    self.updateThemeButtonIcon(initialTheme);
                }
