...
(function() {
    ...
    const ReservationEngine = {
        ...
        checkAvailability: async function(checkin, checkout, roomType) {
            try {
                const response = await fetch(`http://localhost:3001/api/availability?checkin_date=${checkin}&checkout_date=${checkout}&room_type=${roomType}`);
        ...
        handleBooking: function() {
            ...
            if (!formData) {
                console.error('❌ Não foi possível coletar dados do formulário');
                return;
            }
        ...
        showBookingConfirmation: function(data) {
            console.log('🎉 Mostrando confirmação com dados:', data);
            
            const confirmationContainer = document.getElementById('booking-confirmation');
            if (!confirmationContainer) {
                console.error('❌ Container de confirmação não encontrado');
                return;
            }
