...
router.get('/', [
    ...
    try {
        ...
        const availability = {
            ...
            nights,
            available_rooms: {
                individual: rooms.filter(r => r.room_type === 'individual'),
                duplo: rooms.filter(r => r.room_type === 'duplo'),
                triplo: rooms.filter(r => r.room_type === 'triplo'),
                quadruplo: rooms.filter(r => r.room_type === 'quadruplo')
            },
            summary: {
                total_available: rooms.length,
                individual_count: rooms.filter(r => r.room_type === 'individual').length,
                duplo_count: rooms.filter(r => r.room_type === 'duplo').length,
                triplo_count: rooms.filter(r => r.room_type === 'triplo').length,
                quadruplo_count: rooms.filter(r => r.room_type === 'quadruplo').length
            }
        };
